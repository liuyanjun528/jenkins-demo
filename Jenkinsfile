...
node {
    stage('git拉取代码'){
        checkout([$class: 'GitSCM', branches: [[name: "*/${branch}"]], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'git', url: 'git@git.weixin.qq.com:wx_wxa9144934e9688e3b/feynman.git']]])
    }

     stage('maven 打包jenkins-demo'){
            sh "mvn -f feynman-parent clean install -Dmaven.test.skip=true"
        }
